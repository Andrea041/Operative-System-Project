...
/*
 * Stamp  struct sentdata_t
*/
void stampSentdata(sentdata_t data) {
    printf("\tcode: %d\n", data.code);
    printf("\tdate: %s\n", data.date);
    printf("\thour: %s\n", data.hour);
    printf("\tcc: %s\n", data.cc);
...
sendErrorCode:
    // TODO capire che cazzo volevo dare
...
int writeOnBuckup(char* buff) {
    ...
    if(pthread_mutex_unlock(&buckupWrite)) {
        perror("Error unlock backupWrite");
        return -1;
    }
...
int main(int argc, char* argv[]) {
    ...
    while(1) {
        ...
        // Set socket timeout to 300 seconds
        timeout.tv_sec = 300;
        timeout.tv_usec = 0;
